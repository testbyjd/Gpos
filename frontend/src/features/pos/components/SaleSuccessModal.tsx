...
import { getSale, type SaleDetail } from "@/lib/admin-api";
import { getErrorMessage } from "@/lib/api";
import { openCashDrawer } from "@/lib/print-bridge";
...
export function SaleSuccessModal({ sale, onClose }: Props) {
  ...
  // Every completed sale: open cash drawer (bridge). Receipt is manual via Print.
  useEffect(() => {
    let alive = true;
    openCashDrawer().then((res) => {
      if (!alive) return;
      if (!res.ok) setDrawerNote(res.message);
    });
    return () => {
      alive = false;
    };
  ...
  async function handlePrint() {
    ...
    setPrintError(null);
    try {
      const res = await getSale(sale.saleId);
      setPrintSale(res.data);
    } catch (err) {
      setPrintError(getErrorMessage(err, "Receipt load nahi hui."));
    } finally {
      setPrinting(false);
    }
  }
  ...
  return (
    <>
        ...
        >
          ...
          {showChange ? (
            <div className="mx-6 mt-4 rounded-lg border border-success/30 bg-success/10 px-4 py-4 text-center">
              <p className="text-xs font-bold uppercase tracking-wide text-success">Return to customer</p>
              <p className="mt-1 text-4xl font-black tabular-nums text-success">{formatMoney(sale.change)}</p>
            </div>
          ) : isKhata ? (
            <div className="mx-6 mt-4 rounded-lg border border-warning/30 bg-warning/10 px-4 py-3 text-center">
              <p className="text-xs font-bold uppercase tracking-wide text-warning">Added to Khata</p>
          ...
          {drawerNote && (
            <p className="mx-6 mt-3 rounded-md border border-warning/30 bg-warning/10 px-3 py-2 text-xs font-semibold text-warning">
              {drawerNote}
            </p>
          )}
          {printError && (
            <p className="mx-6 mt-2 rounded-md border border-danger/30 bg-danger/10 px-3 py-2 text-xs font-semibold text-danger">
              {printError}
            </p>
          )}
          ...
          <div className="p-6 pt-4">
            <div className="grid grid-cols-2 gap-2">
              <Button size="lg" variant="secondary" className="w-full" onClick={handlePrint} disabled={printing}>
            ...
            <p className="mt-2 text-center text-[11px] text-muted-foreground">
              Receipt auto-print nahi — Print dabao. Drawer har sale pe open hota hai.
            </p>
      ...
      {printSale && (
        <SaleReceiptPrint sale={printSale} autoPrint onClose={() => setPrintSale(null)} />
      )}
    </>
  );
}
